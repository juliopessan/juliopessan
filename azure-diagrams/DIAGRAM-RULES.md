...
- **Render final obrigatório via o app draw.io instalado (MCP)**: após o diagrama passar na validação estrutural e na revisão visual (screenshot), fazer uma chamada final a `mcp__drawio__create_diagram` com o `<mxGraphModel>` completo e definitivo — essa é a integração oficial que abre o diagrama de forma interativa/editável diretamente no app draw.io do usuário. Não usar o navegador (Browser pane) para isso: não navegar para `https://app.diagrams.net/` nem montar URLs com o XML no fragmento `#R<xml>` — esse método é só um workaround de navegador e não abre no app instalado, além de ter o zoom inicial quebrado nesse ambiente. A chamada ao MCP já conta como validação estrutural (retorna erro se o XML for inválido) *e* como o passo de abertura — não é preciso repetir `create_diagram` duas vezes se o XML não mudou entre a validação e a entrega.
