...
- **Validação estrutural via MCP + abertura no app desktop instalado**: o MCP `drawio` (`https://mcp.draw.io/mcp`) é um servidor remoto — ele valida/processa o XML e retorna erros, mas **não tem acesso ao computador do usuário e não abre nada localmente**, apesar de sua descrição sugerir "cria e exibe" um diagrama interativo. Usar `create_diagram` só para a validação estrutural (passo 4 do pipeline).
  - **Abertura final do arquivo**: se o app desktop draw.io (`draw.io.app`, Electron/diagrams.net desktop) estiver instalado, abrir o `.drawio` salvo diretamente nele via `open -a "draw.io" "<caminho/do/arquivo.drawio>"` (macOS). Verificar antes se está instalado com `ls /Applications | grep -i draw.io`.
  - **Não usar o navegador (Browser pane)** para isso: não navegar para `https://app.diagrams.net/` nem montar URLs com o XML no fragmento `#R<xml>` — é só um workaround que não abre o app instalado e tem o zoom inicial quebrado nesse ambiente.
  - Se o app desktop não estiver instalado, informar ao usuário que o `.drawio` está salvo e pode ser aberto manualmente (dando o caminho do arquivo), em vez de tentar simular abertura via navegador.
